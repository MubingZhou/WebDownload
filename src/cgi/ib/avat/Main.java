...
public class Main {
	...
	public static void main(String[] args) {
		try {
			...
			int port = 7496;
			int clientId = 2;  // a self-specified unique client ID
			...
			if(true) {
				...
				con1.secType("STK");
				con1.currency("HKD");
				ArrayList<Contract> conArr2 = new ArrayList<Contract> ();
				conArr2.add(con1);
				
				AvatUtils.downloadHistorical1MinData_20D(myController, conArr, "20170928", "yyyyMMdd");
				//AvatUtils.preparePrevCrossSectionalAvat2(conArr,"20170928", "yyyyMMdd");
			...
			while(now.before(avatTimePath.get(avatTimePath.size() - 1))) {
				...
				for(int i = 0; i < topMktDataHandlerArr.size(); i++) {
					...
					AvatRecordSingleStock at = new AvatRecordSingleStock(stock, price, prevCloseChgPct, ratio5D, ratio20D, industry);
